...
//                sudoCommand + 'chmod -f g+wx "' + state["pio.service.deployment"].path + '/sync" || true',
	    ...
	    function uploadService (sourcePath, serviceId) {

			var servicePath = resolvedConfig.env.PIO_SERVICES_DIRPATH + "/" + serviceId;
			var serviceSyncPath = servicePath + "/sync";

			return API.Q.nbind(API.PACKAGE.fromFile, API.PACKAGE)(API.PATH.join(sourcePath, "package.json"), {
		...
		function uploadBashOrigin () {
			return uploadService(
				API.PATH.dirname(require.resolve("bash.origin/package.json")),
				"bash.origin"
			);
		}

		function uploadPostdeploy () {
			return uploadService(
				API.PATH.dirname(require.resolve("io.pinf.pio.postdeploy/package.json")),
				"io.pinf.postdeploy"
			);
		}
		...
		function uploadProxy () {
			return uploadService(
				API.PATH.dirname(require.resolve("io.pinf.proxy/package.json")),
				"io.pinf.proxy"
			);
		}

		return uploadBashOrigin().then(function () {

			return uploadPostdeploy().then(function () {
